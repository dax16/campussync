...
mock_history = defaultdict(list)
for hour in range(8, 22):
    weight = 0.3
    if 10 <= hour <= 12: weight = 0.9
    elif 13 <= hour <= 15: weight = 0.85
    elif 16 <= hour <= 18: weight = 0.7
    elif hour < 9 or hour > 20: weight = 0.1
...
def analytics():
    peak_hours = [
        {'hour': f'{h:02d}:00', 'demand': round(mock_history[h] + random.uniform(-0.05, 0.05), 2)}
        for h in range(8, 22)
    ]
    return jsonify({
